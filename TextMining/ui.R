 ...
 #   tags$style(HTML("
   ...
   #   .nav-tabs {font-size: 20px} "))),
    tabPanel(strong("About"), 
             ...
             p("We welcome comments, suggestions, notes on any errors or problems encountered by users. Please contact Olga Scrivner (obscrivn AT indiana DOT edu)"),
             p("Contributions: Jefferson Davis, Irina Trapido, Jay Lee"),
             div(style="margin-bottom:20px",a(href="http://cl.indiana.edu/~obscrivn/", target="_blank", "Olga Scrivner"))
    ),
               ...
               navlistPanel(
                 ...
                 tabPanel(strong("ZOTERO"), 
                          fluidPage(
                            ...
                            fluidRow( 
                              column(5,
                                     tags$h4("Format - CSV file"),
                                     fileInput('file.rdf', 'Choose File(s) in CSV format imported from RDF',multiple=FALSE, accept=c('txt','csv')),
                                     uiOutput("print_name_rdf"),
                                     uiOutput("zotero_slider"),
                                     tags$hr(),
                                     tags$br()#,
                                  # textInput("directory", label = "Type the exact directory name", value = "e.g. /Users/Name/Documents/MyZoteroFolder/")
                            ...
                            fluidRow(
                              column(12,
                                     dataTableOutput("print_table_rdf")    
                                     )
                            ),
                            fluidRow(
                              column(7,
                 ...
                 tabPanel(strong("POS-Tagged Text"),
                          shiny::tags$head(shiny::tags$style(shiny::HTML(
                            "#text { font-size: 15px; height: 300px; overflow: auto; }"
                          ))),
                          fluidPage(
                            fluidRow( 
                              column(6, 
                     tags$h4("Format - word/tag per line"),
                      fileInput('file.tag', 'Choose File(s) in txt format',multiple=FALSE, accept=c('txt')),
                     uiOutput("print_name_tagged"),
                     tags$hr()
                      ),
                     column(6,
                            tags$h4("Extract Data"),
                            radioButtons(
                              'tagged_content', 
                              'Select Type',
                              c(word = 'word',
                     ...
                     fluidRow( 
                       column(12,    
                     div(id = "text", uiOutput("print_tagged")),
                          "tm package - readTagged")
                     )
               )
           )
          )
