...
output$print_name_tagged <- renderPrint({
  if (is.null(input$file.tag)) { return()}
  paste(input$file.tag$name, sep="\n")
}) 
...
zoteroData <- reactive({
 ...
 # if ((is.null(input$directory)) || (is.na(input$directory)))  { return() }
  ...
  filename <- read.csv(input$file.rdf$datapath, header=TRUE, sep=",", stringsAsFactors = FALSE)
  
  year <- filename$Publication.Year
  title <- filename$Title
  author <- filename$Author
  text <- filename$Plain.Content
  withProgress(message = 'Loading CSV',
  ...
  info <- list(text=text,year=year,author=author,title=title)
  return(info)
})

output$zotero_term <-renderUI ({
  list(textInput('zotero_term', "Enter your search terms for Zotero, separated by spaces", placeholder = "e.g., airport security"),
...
extractZoteroTerm <- reactive ({
  if (is.null(input$file.rdf))  { return() }
 # if (is.null(zotero_keywords_submitted())) { return() }
  mytext <- zoteroData()$text#texts[[5]]#zoteroData()$zot.pdf#dir.list
  author <-zoteroData()$author#texts[[2]]
  title <- zoteroData()$title#texts[[1]]
  date <- zoteroData()$year#texts[[3]]
  text.extract <- list()
 ...
 # }
  ...
  for (i in 1:num) {
    ...
    if (length(k)==5) {
      kpaste <- paste(k$contextPre,k$keyword,
                      k$contextPost)
  ...
  text.extract <- unlist(text.extract)
  #x <- input$file.rdf$datapath
  #extract <-  extractZotero(z,query,query2,len,words,author,title, date)#p1,p2)
  #extract <-  extractZotero(z,query,query2,len,words)#p1,p2)
  withProgress(message = 'Preprocessing Query',
               detail = 'Almost done...', value = 0, {
                 for (i in 1:15) {
                   incProgress(1/15)
                   Sys.sleep(0.25)
  ...
  extract <- list(text.extract= text.extract,titles=titles,authors=authors,datetimes=datetimes)
  return(extract)
})


output$print_table_rdf<- renderDataTable({
  if (is.null(input$file.rdf)){ return() }
 # z <- zoteroData()$text#texts[[5]]#zoteroData()$zot.pdf#dir.list
  author <-zoteroData()$author#texts[[2]]
  title <- zoteroData()$title#texts[[1]]
  date <- zoteroData()$year#texts[[3]]
  my_data <- data.frame(date=date, title=title, author = author)
  return(head(my_data))
})
...
#   
#   txt.lines <-
#  # txt.lines <- extractZoteroTerm()$text.extract#zoteroData()$dir.list #directory()
# #  txt.lines <- zoteroData$texts[5]#extractZoteroTerm()$titles#zoteroData()$dir.list #directory()
#   HTML(paste("<br/>", "Names of Extracted Aricles: ",txt.lines, sep="<br/>"))
# }) 

zotero_content <- reactive({
  if (is.null(input$file.rdf))  { return() }
 # if (input$zotero_content=="Extract") {
  txt.lines <-  extractZoteroTerm()$text.extract#[[1]]
 #   txt.lines <- extractZoteroTerm()[[1]]#$[[1]]#extractZoteroTerm()$authors#extractZoteroTerm()$text.extract#zoteroData()$texts[5]#extractZoteroTerm()$text.extract
 # }
  #else if (input$zotero_content=="Full"){
  #  txt.lines <- extractZoteroTerm()$text.full
 # }
 # else if (input$zotero_content=="Abstract"){
  #  txt.lines <- extractZoteroTerm()$abstract
#  }
  return(txt.lines)
})
output$print_zotero <- renderUI({
  if (is.null(input$file.rdf))  { return() }
  # if (input$zotero_content=="Extract") {
  # txt.lines<- extractZoteroTerm()$text.extract
  # }
  # else {
  #   txt.lines<- extractZoteroTerm()$text.full
  # }
...
tagged <- reactive({
  ...
  num <-length(input$file.tag)
    ...
    both <- vector()
 # for (k in 1:num) { 
    uris.name <- input$file.tag$datapath
  ...
  word.paste <- paste(words,collapse=" ")
  pos.paste <- paste(poss,collapse=" ")
  pos[1] <- pos.paste
 # pos <- unlist(pos)
  word[1] <- word.paste
 # word <- unlist(word)
 # }
  info <- list(word=word,pos=pos,both=both,y=y)
  return(info)
})

selectPos <- reactive ({
  if (is.null(input$file.tag)) { return() }
  if (input$tagged_content=="word") {
    tagged.corpus <- tagged()$word
  }
  else if (input$tagged_content=="pos") {
    tagged.corpus <- tagged()$pos
  }
 else  if (input$tagged_content=="both") {
    tagged.corpus <- tagged()$both
  }
  return(tagged.corpus)
})

output$print_tagged <- renderUI({
  if (is.null(input$file.tag)) { return() }
  txt.lines <- selectPos()
  HTML(paste("<br/>", "Document: ",txt.lines, sep="<br/>"))
})
...
fileData <- reactive({ # loading data
  ...
  if( !is.null(input$csv_file_name) ) {
    ...
    a <- extractZoteroTerm()$authors
    t <- extractZoteroTerm()$titles
    dt <- extractZoteroTerm()$datetimes
    my_data <- data.frame(date=dt, title=t, author = a)
  }
...
RemoveWordsStepThree <-reactive({
 # if ((is.null(input$file.article)) && (is.null(input$file.article.txt)) && (is.null(structured_data()))&& (is.null(input$file.tag))&&(is.null(input$file.rdf))) { return() }
  ...
  if(!is.null(input$file.rdf)) {
    file.names <- extractZoteroTerm()$titles
  }
...
LengthWord <- reactive ({
  ...
  if (!is.null(structured_data())) {
      names <- structured_data()$titles
    n <- which(names==i)
    vec <- structured_data()$corpus[[n]]
  }
  if (!is.null(input$file.rdf)) {
    names <- extractZoteroTerm()$titles
    n <- which(names==i)
    vec <- zotero_content()[[n]]#extractZoteroTerm()$text.extract[[n]]
  }
...
LengthSent <- reactive ({
  ...
  if (!is.null(structured_data())) {
    names <- structured_data()$titles
    n <- which(names==i)
    data <- structured_data()$corpus[[n]]
  }
  if (!is.null(input$file.rdf)) {
    names <- extractZoteroTerm()$titles
    n <- which(names==i)
    data <- zotero_content()[[n]]#extractZoteroTerm()$text.extract[[n]]
  }
...
LdaAnalysis <- reactive({
  ...
  if(!is.null(input$file.rdf)) {
    corpus.lda <-  RemoveWordsStepOne()$corpus
    n.docs <- as.numeric(length(extractZoteroTerm()$titles))
  }
  if(!is.null(structured_data())) {
    corpus.lda <-  RemoveWordsStepOne()$corpus
...
output$heatmap_punct <- renderPlot({
  ...
  if (!is.null(input$file.rdf)) {
    text <-zotero_content()#extractZoteroTerm()$text.extract
  }
  if (!is.null(structured_data())) {
    text <- length(structured_data()$corpus)
  }
  heatmap(text)
})
