...
structured_data <- reactive({ # loading data
  if ((!is.null(input$file.article)) || (!is.null(input$file.article.txt)) || (!is.null(input$file.tag))) { return() }
  parsed_data = NULL
...
output$choose_term <- renderUI({
  if ((is.null(input$file.article)) && (is.null(input$file.article.txt)) && (is.null(structured_data()))&& (is.null(input$file.tag)))   { return() } 
...
output$zotero_term <- renderUI({
  ...
  selectizeInput("zotero_term", label = "Type term", 
                 #  choices = word,
                 options = list(create = TRUE),
                 selected = NULL,
                 multiple = TRUE) 
}) 
output$pattern1 <- renderUI({
  textInput("pattern1", label = h3("Type the first term"), value = "Enter text...")
  
})

output$pattern2 <- renderUI({
  textInput("pattern2", label = h3("Type the second term"), value = " ")
...
zoteroData <- reactive ({
  if (is.null(input$file.rdf))  { return() }
  # extract <- zotero(input$file.rdf$datapath)
  # roots = c(wd='.')
  ...
  #  zot.pdf <- paste(zot.lines, collapse=" ")
  # zot.pdf <- zotero(zot.lines)
  #zot.line.parser
  # zot.pdf <- unlist(lapply(zot.lines, zot.line.parser))
  # pattern1 <- input$pattern1
  # pattern2 <- input$pattern2
  #  extract <-extractZotero(z,pattern1,pattern2)
  return(zot.pdf)
})

output$print_content_rdf <- renderUI({
  if (is.null(input$file.rdf))  { return() }
  # if ((is.null(input$pattern1)) || (is.null(input$pattern2))) { return() }
  txt.lines <-  zoteroData()
  # paste(txt.lines, collapse=" ")
...
#  p1 <- input$pattern1
 # p2 <- input$pattern2
  ...
  #name <-input$file.rdf$name
  #d2 <-input$file.rdf #getwd()
  #d <- gsub(name,"",d2)
  # uris.name<- paste0(dirfile,uris)
  # dirFile <- paste0()
  y=input$file.rdf$datapath
  extract <-  extractZotero(z,y)#,p1,p2)
  return(extract)
})

output$print_zotero <- renderUI({
  if (is.null(input$file.rdf))  { return() }
 # if ((input$pattern1=="") && (input$pattern2=="")) { return() }
 # if ((is.null(input$pattern1)) || (is.null(input$pattern2))) { return() }
...
output$choose_length <- renderUI({
  # if ((is.null(input$file.article)) && (is.null(input$file.article.txt)) && (is.null(structured_data())))   { return() } 
  # d <- ListTerms()$d
  selectizeInput("len",
                 "Context Length:",
